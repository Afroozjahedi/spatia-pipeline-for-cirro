...
- **`_latest_stats_csv(log_dir)`** — globs `processing_stats_*.csv` and returns the lexically-last one (the timestamp format sorts correctly as a string). Since the auto-hook calls this *within the same `run_preprocessing()` call* that just wrote that run's stats CSV, it always reads that run's own file — no ambiguity there. For a later manual CLI re-render, if `run_preprocessing()` has been run more than once for this config, this still always reads the *most recent* run's stats, not a merge of all runs.
